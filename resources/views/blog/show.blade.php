	    ...
	    <div class="panel">
	        ...
	        <div class="panel-heading">
	            ...
	            <div class="author text-center">
                    ...
                    <a href="{{ route('users.show', $blog->user->username) }}" class="author">{{ $blog->user->name() }}</a>
	                <span class="">
	                	{{ Date::parse($blog->created_at)->format('d M Y') }}
	                </span>
	                ...
	                <div class="action-buttons">		     
				      @if(isAdmin())
				      	...
				      	<?php 
				      	$class = $blog->highlighted ? 'yellow' : 'white';
				      	$title = $blog->highlighted ? 'Highlighted' : 'Not Highlighted';
				      	?>
				        {{ link_to_route('blog.highlight', $title, array($blog->id), 
				          array('class' => 'btn '.$class)) }}

				      	<?php 
				      	$class = $blog->top_story ? 'blue' : 'white';
				      	$title = $blog->top_story ? 'Top Story' : 'Not Top Story';
				      	?>
				        {{ link_to_route('blog.top', $title, array($blog->id), 
				          array('class' => 'btn '.$class)) }}
				      ...
				      @if(canEdit($blog))
				        {{ link_to_route('blog.edit', 'Edit', array($blog->slug), 
	        ...
	        <div class="panel-body">
				...
				<hr/>
		         ...
		         <div class="sharer">
					<?php 
					$url = Request::url();
					$title = trim(preg_replace('/\s\s+/', ' ', $blog->title));
					$stars = count($blog->stars);
					?>

					@if(isset($auth))
						@if($auth->starred($blog))
					    <a href="{{ route('blog.unstar', $blog->slug) }}" title="UnStar blog">
					        <div class="star starred"><i class="icon-star"></i> {{ $stars }}</div>
					    </a>
					    @else
					    <a href="{{ route('blog.star', $blog->slug) }}" title="Star blog">
					        <div class="star"><i class="icon-star"></i> {{
					         $stars }}</div>
					    </a>
					    @endif
				    @else
				    <a href="#signin" data-toggle="modal" title="UnStar blog">
				        <div class="star starred"><i class="icon-star"></i> {{ $stars }}</div>
				    </a>
				    @endif
    ...
    <script type="text/javascript">
		function social_popup(mode, url, text) {
			...
			switch (mode) {
				...
				break;
				case 'ln':
				url = 'http://www.linkedin.com/shareblog?mini=true&url=' + url;
				break;
			}
		...
		// function starblog(blog_id, user_id) {
